...
export const STEPS = {
  1: {
    ...
    body: `Nothing is running yet. The client writes down what it wants — the function, the argument,
              ...
              not the platform's, and it is where step 32 delivers.
              ...
              create hangs off it: the child in step 7 is <code>foo.1:1</code>.</span>`,
  },
  2: {
  ...
  7: {
    ...
    body: `The rpc call does not invoke <code>bar</code>. It asks for a promise — <code>foo.1:1</code> —
              ...
              which is exactly why the replay in step 25, running the same line for the second time,
  ...
  9: {
    ...
    body: `The server records <code>foo.1:1</code> and — because it now has a promise with a target and
              no result — has a second piece of work to schedule. That is the dispatch in step 14.`,
  },
  10: {
    lines: [2],
    wire: `promise.create res foo.1:1, pending`,
  ...
  12: {
    ...
    body: `The claim goes back to the server. From here until step 21 <em>no process anywhere is
  ...
  16: { wire: `task.acquire res foo.1:1, preload=[]`, title: "Claimed", body: `Worker B holds <code>foo.1:1</code>.` },
  17: {
    lines: [7, 8],
  ...
  24: {
    ...
    body: `Worker B starts <code>foo</code> at its entry point with <code>n = 5</code> — the same way
               Worker A did in step 6. No stack is restored and no coroutine is resumed. The function
  ...
  26: {
    ...
    body: `Byte for byte the request Worker A sent in step 8, under a different claim —
